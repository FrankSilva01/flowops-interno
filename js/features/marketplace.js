...
(function(){function patch(){try{var S=window.supabase;if(!S||!S.createClient)return false;var c=S.createClient('https://example.supabase.co','anon');var b=c.from('_').select();var p=Object.getPrototypeOf(b);while(p&&!Object.getOwnPropertyDescriptor(p,'then'))p=Object.getPrototypeOf(p);if(p&&typeof p.then==='function'&&typeof p.catch!=='function'){Object.defineProperty(p,'catch',{value:function(f){return this.then(undefined,f);},writable:true,configurable:true});Object.defineProperty(p,'finally',{value:function(f){return this.then(function(v){if(f)f();return v;},function(e){if(f)f();throw e;});},writable:true,configurable:true});}return true;}catch(e){return false;}}if(!patch()){var n=0,id=setInterval(function(){if(patch()||++n>50)clearInterval(id);},100);try{window.addEventListener('DOMContentLoaded',patch);window.addEventListener('load',patch);}catch(e){}}})();
