...
async function downloadMlDocument(
  ...
  const archive = await archiveMarketplaceDocument({
    organizationId: account.organization_id,
    marketplace: "Mercado Livre",
    externalOrderId: orderId,
    internalOrderId: link.internal_order_id,
    documentType,
    externalDocumentId: shippingId,
    fileName,
...
async function uploadMlPicture(pic: unknown, account: Record<string, any>, index: number) {
  if (typeof pic !== "string") throw new Error("Imagem invalida.");
  if (/^https?:\/\//i.test(pic)) return { source: pic };
  if (!pic.startsWith("data:image/")) throw new Error("Imagem invalida.");

  const blob = dataUrlToBlob(pic);
  const form = new FormData();
  const extension = blob.type.includes("png") ? "png" : "jpg";
  form.append("file", blob, `produto-${index + 1}.${extension}`);
  ...
  if (!uploadResp.ok) {
    throw new Error(uploadData.message || uploadData.error || "Falha ao enviar imagem para o Mercado Livre.");
  }
  const secureUrl = uploadData.secure_url || uploadData.variations?.[0]?.secure_url || uploadData.url;
  if (!secureUrl) throw new Error("Mercado Livre nao retornou URL da imagem.");
...
async function createMlListing(body: Record<string, any>, account: Record<string, any>, actorEmail: string) {
  ...
  pictures.push(...await Promise.all(picturesToProcess.map((pic, index) => uploadMlPicture(pic, account, index))));
  for (const pic of []) {
    try {
      let uploadUrl = pic;
