...
const DEFAULT_RESPONSIBLES = ["Franklin", "Ravena", "Anderson", "Dayane"];
const PRODUCTION_STAGES = ["Em fila", "Imprimindo", "Pós-processo", "Pintando", "Pronto", "Entregue"];
const PRIORITY_OPTIONS = ["", "Baixa", "Normal", "Alta", "Urgente"];
const STATUS_OPTIONS = ["Orçamento", "A preparar", "Despachado", "A caminho", "Entregue"];
const SUBSCRIPTION_DEFAULT_GRACE_DAYS = 5;
const state = {
  view: getInitialView(),
  query: "",
  data: loadData(),
  ...
  organizationId: "",
  organizationSlug: "",
  organizationName: window.SUPABASE_CONFIG?.COMPANY_NAME || "3D.AFT",
  supportMode: false,
  isAdmin: false,
  canEdit: true,
  accessRequests: [],
...
async function resolveLoginBrand() {
...
async function signupOnline(event) {
  event.preventDefault();
  const message = byId("onlineSignupMessage");
  message.textContent = "";
  message.classList.remove("success");
  const name = byId("signupName").value.trim();
  const email = byId("signupEmail").value.trim().toLowerCase();
  const password = byId("signupPassword").value;
  if (await emailAlreadyApproved(email)) {
    message.textContent = "Este e-mail ja tem acesso ativo. Enviando link de recuperacao de senha...";
    try {
      await sendPasswordRecovery(email);
      message.textContent = "Este e-mail ja existe nos usuarios ativos. Enviamos um link de recuperacao de senha.";
    } catch (error) {
      message.textContent = `Este e-mail ja existe nos usuarios ativos, mas nao consegui enviar recuperacao: ${error.message}`;
    }
    return;
  }
  try {
    await userAccessRequest({
      ...
      email,
      password,
      organization_id: window.SUPABASE_CONFIG?.ORGANIZATION_ID || "00000000-0000-0000-0000-000000000001"
    });
  } catch (error) {
    if (error.code === "already_approved") {
      message.textContent = "Este e-mail ja possui acesso. Use Recuperar senha para redefinir a senha.";
      return;
    }
    message.textContent = `Nao foi possivel solicitar cadastro: ${error.message}`;
    return;
  }
  message.textContent = "Cadastro solicitado. O administrador foi avisado e poderá aprovar ou recusar o acesso.";
  message.classList.add("success");
  event.currentTarget.reset();
}

async function userAccessRequest(payload, requireSession = false) {
  const config = window.SUPABASE_CONFIG || {};
  const headers = {
...
async function recoverPasswordFromLogin() {
...
async function emailAlreadyApproved(email) {
  try {
    const { data, error } = await state.supabase.rpc("is_email_approved", { candidate_email: email });
...
async function saveAccessRequest(name, email) {
  try {
    const { error } = await state.supabase.from("access_requests").insert({
      email,
      name,
      status: "pending",
    ...
    console.warn("Nao foi possivel registrar a solicitacao em access_requests.", error);
    return false;
  }
}

async function enterOnlineApp(user) {
...
async function chooseMembership(memberships) {
...
async function canUserAccess(user) {
...
async function getApprovedUser(email) {
  try {
    const { data, error } = await state.supabase
      .from("approved_users")
...
function setSessionInfo(name, role, mode, canLogout) {
...
async function removeRemote(kind, id) {
  if (!state.online || !state.supabase) return;
...
async function loadRemoteData() {
  ...
  await repairKnownCashDuplicate();
}

async function repairKnownCashDuplicate() {
  const obsolete = state.data.cash.find((entry) =>
    /^PED-0022\s*-\s*Taça Copa$/i.test(String(entry.description || "").trim())
    && Number(entry.income || 0) === 30
  );
  const canonical = state.data.cash.find((entry) =>
    /^PED-0023\s*-\s*Taça Copa$/i.test(String(entry.description || "").trim())
    && Number(entry.income || 0) === 70
  );
  if (!canonical) return;
  const obsoleteOrder = state.data.orders.find((order) =>
    getOrderCode(order) === "PED-0022"
    && /^Taça Copa$/i.test(String(order.description || "").trim())
  );
  if (!obsolete && !obsoleteOrder) return;
  if (obsolete) state.data.cash = state.data.cash.filter((entry) => entry.id !== obsolete.id);
  if (obsoleteOrder) state.data.orders = state.data.orders.filter((order) => order.id !== obsoleteOrder.id);
  if (state.supabase) {
    if (obsolete) {
      const { error } = await state.supabase.from("cash_entries").delete().eq("id", obsolete.id);
      if (error) console.warn("Não foi possível remover o lançamento duplicado legado.", error);
    }
    if (obsoleteOrder) {
      const { error } = await state.supabase.from("orders").delete().eq("id", obsoleteOrder.id);
      if (error) console.warn("Não foi possível remover a encomenda duplicada legada.", error);
    }
  }
}
...
async function seedRemoteData() {
  const results = await Promise.all([
    ...INITIAL_DATA.orders.map((item) => state.supabase.from("orders").upsert(toRemote("orders", item))),
    ...INITIAL_DATA.cash.map((item) => state.supabase.from("cash_entries").upsert(toRemote("cash", item)))
  ]);
...
function fromRemoteMaterial(item) {
  return {
    id: item.id,
    date: item.date,
    supplier: item.supplier,
...
function renderBarChart(elementId, rows) {
...
function renderColumnChart(elementId, rows) {
  renderLineChart(elementId, rows.map((item) => ({ label: formatDateShort(item.date), value: Number(item.income || 0) - Number(item.expense || 0) })), { valueLabel: "Movimento" });
}
...
function getChecklistProgress(item) {
  const checklist = { ...defaultChecklist(), ...(item.checklist || {}) };
  const keys = Object.keys(checklist);
  return {
    done: keys.filter((key) => checklist[key]).length,
    total: keys.length
  };
}
...
function flashActionMessage(message) {
...
async function handlePaymentAction(action) {
  ...
  try {
    ...
    if (action === "reconcile-billing") {
      ...
      showToast("Cobrança e assinatura atualizadas.");
      await loadRemoteData();
      return;
    }
  } catch (error) {
...
function showAppMessage(title, message, tone = "info") {
